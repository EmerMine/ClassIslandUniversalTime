...
2. 转到 [Releases](https://github.com/EmerMine/ClassIslandUniversalTime/releases/) 下载压缩包并解压到一个合适的目录下。
3. 新建管理员终端，工作目录设置为当前脚本所在目录。
4. 运行 `pwsh.exe .\UniversalTime.ps1 -TestCompensation -GUI` 测试补偿值（补偿值因设备而异）。
5. 运行 `pwsh.exe .\ClassIslandConnecter.ps1` 将 Windows 时间设置为 ClassIsland 偏移后的时间。
6. （推荐）在 ClassIsland 插件页面安装 `以管理员身份自启动` 插件。
7. （推荐）在 ClassIsland 自动化内设置启动 ClassIsland 时自动运行 `pwsh.exe path\to\ClassIslandConnecter.ps1` 同步偏移后的时间。
